...
class ToricCode:
    ...
    def __init__(self, L):
        ...
        self.L = L
        # qubits where an X error occured
        self.Xflips = np.zeros((2*L, L), dtype=np.dtype('b'))
        # qubits where a  Z error occured
        self.Zflips = np.zeros((2*L, L), dtype=np.dtype('b'))
    ...
    def flatZflips2Xstab(self):
        ...
        for i, j in itertools.product(range(L), range(L)):
            _flatZflips2Xstab[(i+1) % L*L+(j+1) %
                              L, (2*i+1) % (2*L)*L+(j+1) % L] = 1
            _flatZflips2Xstab[(i+1) % L*L+(j+1) %
                              L, (2*i+2) % (2*L)*L+(j) % L] = 1
            _flatZflips2Xstab[(i+1) % L*L+(j+1) %
                              L, (2*i+3) % (2*L)*L+(j+1) % L] = 1
            _flatZflips2Xstab[(i+1) % L*L+(j+1) %
                              L, (2*i+2) % (2*L)*L+(j+1) % L] = 1
    ...
    def Zstabilizer(self):
        ...
        stab = self._Zstab
        X = self.Xflips
        stab[0:-1, 0:-1] = X[0:-2:2, 0:-1:] ^ X[1:-1:2,
                                                0:-1:] ^ X[2::2, 0:-1:] ^ X[1:-1:2, 1::]
        stab[-1, 0:-1] = X[-2, 0:-1:] ^ X[-1, 0:-1:] ^ X[0, 0:-1:] ^ X[-1, 1::]
        stab[0:-1,  -1] = X[0:-2:2,  -1] ^ X[1:-
                                             1:2,  -1] ^ X[2::2,  -1] ^ X[1:-1:2,  0]
    ...
    def Xstabilizer(self):
        ...
        stab = self._Xstab
        Z = self.Zflips
        stab[1:, 1:] = Z[1:-2:2, 1:] ^ Z[2:-1:2,
                                         0:-1] ^ Z[3::2, 1:] ^ Z[2:-1:2, 1:]
    ...
    def Zcorrections(self):
        ...
        for (y1, x1), (y2, x2) in matches:
            ...
            qubits.update(((_+1) % (2*L), vertical % L)
                          for _ in range(ym, yM, 2))
    ...
    def Xcorrections(self):
        ...
        for (y1, x1), (y2, x2) in matches:
            ...
            if xM-xm > L/2:
                ...
                vertical = xm
            qubits.update(((horizontal+1) % (2*L), (_+1) % L)
                          for _ in range(xm, xM))
            qubits.update(((_+2) % (2*L), vertical % L)
                          for _ in range(ym, yM, 2))
        return matches, qubits
...
def stat_estimator(samples, cutoff=200, confidence=0.99):
    ...
    def y(c): return N**0.5*(estimate/c-1)*(1-np.exp(-c*cutoff))**0.5
    ...
    if not (low.success and high.success):
        raise RuntimeError(
            'Could not find confidence interval for the given samples!')
...
def find_threshold(Lsmall=3, Llarge=5, p=0.8, high=1, low=0.79, samples=1000, logfile=None):
    ...
    def step(p):
        ps.append(p)
        samples_small.append(stat_estimator(
            sample(Lsmall, p, samples=samples)))
        samples_large.append(stat_estimator(
            sample(Llarge, p, samples=samples)))
    ...
    if logfile:
        with open(logfile, 'w') as f:
            ss = samples_small[0]
            sl = samples_large[0]
            f.write(str((np.vstack([ps, [ss[0]], [ss[1]-ss[0]], [ss[2]-ss[0]],
                    [sl[0]], [sl[1]-sl[0]], [sl[2]-sl[0]]]), (ss[0]+sl[0])/2, ps[0])))
    else:
        f = plt.figure()
        s = f.add_subplot(1, 1, 1)
    while not (samples_large[-1][1] < samples_small[-1][0] < samples_large[-1][2]
        ...
        ix, iy = intersection(
            ps[-2:], [_[0] for _ in samples_small[-2:]], [_[0] for _ in samples_large[-2:]])
        if logfile:
            with open(logfile, 'w') as f:
                f.write(
                    str((np.vstack([_ps, _small, _small_err, _large, _large_err]), iy, ix)))
        else:
            s.clear()
            s.errorbar(_ps, _small, yerr=_small_err,
                       alpha=0.6, label=str(Lsmall))
            s.errorbar(_ps, _large, yerr=_large_err,
                       alpha=0.6, label=str(Llarge))
...
# TODO duplicated code with data_generator in neural.py
